...
int main(int argc, char** argv) {
    ...
    try{
        ...
        if(programs.size() > 1) {
			// high_resolution_clock::time_point t1_hash = high_resolution_clock::now();
            // ::hash(programs);
			// high_resolution_clock::time_point t2_hash = high_resolution_clock::now();
            // auto duration_hash = duration_cast<milliseconds>(t2_hash - t1_hash).count();
            // cout << "Duration of hash: " << duration_hash << "[ms]" << endl;
            ...
            ::hash(programs);
            pp2 = GST_mod::comparePthreads(programs);
            high_resolution_clock::time_point t4 = high_resolution_clock::now();

            auto duration2 = duration_cast<milliseconds>(t4 - t3).count();

            cout << "Duration of method2: " << duration2 << "[ms]" << endl;
