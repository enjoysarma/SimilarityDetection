...
/**
 * Optimizacija: Dodaj parametar M-za moduo. Njega ces kao masku da primenis na izracunatu hash vrednost
 * Na ovaj nacin se umanjuje zauzece memorije koriscenjem manjeg broja bita za smestanje hesiranih vrednosti
 */
unsigned long long KarpRabin::init_hash(vector<Token*>& tokens) {
...
void KarpRabin::hash(Program *p) {
	...
	vector<unsigned long long>& hashes 	= p->getKgramHashes();
	//clear any previous hashes
	hashes.clear();

	unsigned int mml 			= Parameters::getMinimumMatchLength();
	unsigned int b 				= Parameters::getHashBase();
	unsigned long long 	base	= pow(b, mml - 1);
	unsigned long long  hash 	= init_hash(tokens);
